...
if __name__ == '__main__':
    ...
    img_gt = cv2.imread(r'E:\DIV2K\DIV2K_train_HR\0001.png') / 255.

    # -------------- cv2 -------------- #
    encode_param = [int(cv2.IMWRITE_JPEG_QUALITY), 20]
    _, encimg = cv2.imencode('.jpg', img_gt * 255., encode_param)
    img_lq = np.float32(cv2.imdecode(encimg, 1))
    cv2.imwrite('E:\DIV2K\cv2_JPEG_20.png', img_lq)

    # -------------- DiffJPEG -------------- #
    jpeger = DiffJPEG(differentiable=False).cuda()
    img_gt = img2tensor(img_gt)
    img_gt = torch.stack([img_gt, img_gt]).cuda()
    quality = img_gt.new_tensor([80, 40])
    out = jpeger(img_gt, quality=quality)

    cv2.imwrite('E:\DIV2K\pt_JPEG_80.png', tensor2img(out[0]))
    cv2.imwrite('E:\DIV2K\pt_JPEG_40.png', tensor2img(out[1]))